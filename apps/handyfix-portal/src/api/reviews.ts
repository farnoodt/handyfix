...
export async function createReview(jobId: string, req: { rating: number; comment?: string | null }) {
  return unwrap<ReviewDto>(http.post<ApiResponse<ReviewDto>>(`/jobs/${jobId}/reviews`, req));
}

export async function getReview(jobId: string) {
  return unwrap<ReviewDto | null>(http.get<ApiResponse<ReviewDto | null>>(`/jobs/${jobId}/reviews`));
}
