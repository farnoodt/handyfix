...
export default function BookJob() {
  ...
  const [items, setItems] = useState<servicesApi.ServiceItemDto[]>([]);
  ...
  useEffect(() => {
    (async () => {
      const i = await servicesApi.getItems();
      setItems(i);
    })();
  }, []);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setErr(null);
    if (!isAuthed) { setErr("Please login first."); return; }
    ...
    <div>
      ...
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 10, maxWidth: 640 }}>
        <label>
          ...
          <select value={serviceItemId} onChange={(e) => setServiceItemId(e.target.value === "none" ? "none" : Number(e.target.value))}>
